...
[CF 104288C - 公正部門](https://codeforces.com/problemset/problem/104288/C)
...
もし$x_{n+1}=0$、のハミング重み$x$の重さに等しい$(x_1,\dots,x_n)$、つまり、最初のインデックスのうち選択されたインデックス$n$座標は変わりません。 これにより、$$h_{n+1}(x_1,\dots,x_n,0)=h_n(x_1,\dots,x_n).$$もし$x_{n+1}=1$、体重は増加します$1$したがって、選択されたインデックスは 1 位置ずつシフトし、変数の効果的な順序がパート (a) のように回転されます。 したがって、関数は回転されたタプルに作用します$(x_2,\dots,x_n,x_1)$:$$h_{n+1}(x_1,\dots,x_n,1)=h_n(x_2,\dots,x_n,x_1).$$両方の場合を組み合わせると、$$h_{n+1}(x_1,\dots,x_{n+1})=(x_{n+1} ? h_n(x_2,\dots,x_n,x_1) : h_n(x_1,\dots,x_n)).$$### (c)

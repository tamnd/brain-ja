...
4. 同様にプレフィックスのハッシュを生成します。 それぞれのハッシュはソートされたサフィックス配列とすぐに照合できるため、すべての結合されたプレフィックス ハッシュを保存する必要はありません。 
...
6. 最小のサフィックスハッシュを考慮します。 もし`X + smallest_suffix < MOD`、このプレフィックスに対してラップされていない最良の結果が得られます。 式のサフィックス ハッシュが増加するため、サフィックスを大きくしても非ラッピングの結果を改善することはできません。 
7. を満たす最初のサフィックスハッシュを見つけます。`suffix >= MOD - X`。 このような値が存在する場合、最適なラッピング結果が得られます。 繰り返しますが、式をラップした後は、後続のサフィックス ハッシュごとに、少なくとも同じ大きさの結果が生成されます。`X + suffix - MOD`。 
...
| 変数 | フクロウ | ヤギ |
 ...
 この例の重要な部分は、アルゴリズムがすべて (3^6=729) の完全な文字列を構築することはないということです。 27 個のハーフハッシュの 2 セットを構築し、モジュラー不等式を通じてそれらを結合するだけです。 

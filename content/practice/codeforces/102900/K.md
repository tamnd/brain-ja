...
[CF 102900K - 旅商人](https://codeforces.com/problemset/problem/102900/K)
...
させて$k$に等しい部分の数を示します$a_1$。 それから$m-k$部分が等しい$a_1 - 1$、和の条件は次のようになります。$$n = k a_1 + (m-k)(a_1 - 1).$$展開すると得られる$$n = k a_1 + m a_1 - k a_1 - m + k = m(a_1 - 1) + k.$$したがって、$$k = n - m(a_1 - 1).$$以来$0 \le k \le m$、整数$a_1$によって制約されています$$m(a_1 - 1) \le n \le m(a_1 - 1) + m,$$これは以下と同等です$$a_1 - 1 \le \frac{n}{m} \le a_1.$$したがって$a_1 = \left\lceil \frac{n}{m} \right\rceil$。 書き込み$n = qm + r$と$0 \le r < m$、これは次の結果をもたらします$q = \left\lfloor \frac{n}{m} \right\rfloor$そして$$a_1 =
\begin{cases}
q, & r = 0,\\
q+1, & r > 0.
\end{cases}$$もし$r = 0$、 それから$k = n - m(q-1) = mq - m(q-1) = m$、つまりすべての部分が等しい$q$。 

もし$r > 0$、 それから$a_1 = q+1$、 そして$$k = n - m q = r.$$したがって、まさに$r$部分が等しい$q+1$、残りは$m-r$部分が等しい$q$。 

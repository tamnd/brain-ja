...
A B TRUCK 3
A B
```答えは`3`。 交通手段を含めずに 2 つの都市名間のエッジを 1 つだけ保存する不用意なグラフ表現は、より安価な、またはより関連性の高い状態を上書きする可能性があります。 
...
A B TRUCK 3
A B
```答えは`3`、パッケージは出発地でトラックを選択するだけでよいためです。 より一般的には、宛先は、最初に選択されたモードの最終ルート セグメントで使用されるモードだけでなく、4 つのモード状態すべてで成功したと見なされなければなりません。 
...
3. 路線区間ごとに`(u, v, mode, cost)`、 接続する`(u, mode)`そして`(v, mode)`指定されたコストの無向エッジを使用します。 このモードは州の一部であるため、空路で使用できるルートを誤って鉄道やトラックで使用できるようにしてはなりません。 
...
の`state`関数マップ`(city, mode)`に`city * 4 + mode`。 これにより、すべての状態に一意のインデックスが与えられます。`0`を通して`4 * city_count - 1`。 固定係数 4 により、モード遷移の構築も簡単になります。 

6 つのスイッチング エッジはルート エッジの前に作成されます。 スイッチングコストがかかる都市の場合`c`、異なるモードのすべてのペアに重みのエッジが与えられます。`c`。 この問題のルールでは、航空から鉄道への変更は鉄道から航空への変更と同じコストがかかるため、グラフに方向性はありません。 
...
B C RAIL 3
A C
```の初期状態`A`すべての距離はゼロです。 エアエッジが与える`(B, AIR)`距離`4`。 で`B`、からのスイッチングエッジ`(B, AIR)`に`(B, RAIL)`費用`2`、距離を生み出す`6`。 レールの端が到達します`(C, RAIL)`遠くで`9`。 目的地の最小距離は次のとおりです。`9`。 状態表現により、スイッチング電荷が正確に正しい点に表示されます。 
...
最後に、送信元と送信先には必須の受信モードまたは送信モードがないため、特別な処理が必要です。 4 つの原点状態をすべてゼロに初期化すると、最初のモードを自由に選択できるようになります。 4 つの目的の状態すべてで最小値を取ると、どのモードでも終了できる自由がモデル化されます。 どちらかの側を 1 つの任意のモードに制限すると、別の問題が解決され、より大きな答えが得られる可能性があります。

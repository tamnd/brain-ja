...
2 番目のエッジ ケースは上限です。 のために`a = 15, b = 15`、答えには次のものが含まれます`15`それ自体、なぜなら`15`で割り切れます`3`そして`5`。 のみに基づいた式`b // d // 2`奇数の倍数をカウントするときに、誤ってこの値を失う可能性があります。 奇数の約数の正しい奇数倍数`d`まで`b`は`(b // d + 1) // 2`。 
...
この関数には標準的な繰り返しがあります。 最初の回避回数がすでにわかっている場合`k-1`素数の場合、それらの数の中から次の数で割り切れるものを削除します。`pk`。 このような数値をで割った後、`pk`、残るのは正確に、によって数えられたセットです。`phi(b // pk, k-1)`。 したがって、`phi(x, k) = phi(x, k-1) - phi(x // pk, k-1)`。 
...
| アプローチ | 時間計算量 | 空間の複雑さ | 評決 |
 ...
 1. すべてのクエリを読み取り、以下の素数のリストを決定します。`130`。 お問い合わせの場合`(a, b)`、素数のみ`p <= a`大事なことだから、そうしましょう`k`彼らの数になってください。 これにより、元の可能な約数の範囲が変換されます。`2..a`せいぜい`31`明確な最高の条件。 
2. 定義する`phi(x, k)`整数の数として`[1, x]`最初のどれでも割り切れないもの`k`素数。 望ましい答えは、`x - phi(x, k)`なぜなら、すべての正の整数は、これらすべての素数に触れていないか、少なくとも 1 つの素数で割り切れるためです。 
...
の`small_phi`テーブルはあらゆる状態を処理します`x < 800`。 その反復は数学的定義とまったく同じであるため、近似やヒューリスティックではありません。 このテーブルは、繰り返しの再帰を定数時間の検索に置き換えるだけです。 
...
| スペース |`O(800 * 31 + S)`| 固定の小さなテーブルは約 25,000 の整数を使用しますが、`S`キャッシュされた大きいサイズの数です`(x,k)`状態 |
 ...
 キー制約はそうではありません`b`それ自体、それ以来`b`到達できる`10^15`。 アルゴリズムがその範囲を反復することはありません。 その仕事は以下の少数の素数によって決まります。`130`そして、整数の除算によって生成される個別の部分ふるい状態によるものです。 固定素数境界により、この方法が実用的になります。 
